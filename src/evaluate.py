...
def _bootstrap_ci(values, stat_fn, n_boot=2000, ci=0.95, seed=42):
    """Bootstrap confidence interval for a statistic on a 1-D array."""
...
def evaluate_model(model, test_dataset, device, output_dir='outputs', batch_size=8):
    """
    ...
    # Correlation
    correlation = np.corrcoef(all_gt_counts, all_pred_counts)[0, 1] if len(all_gt_counts) > 1 else float('nan')
    ...
    # Per-quartile breakdown — reveals if model struggles at extreme counts
    quartile_stats = []
    if len(all_gt_counts) >= 8:
        ...
        for q in range(4):
            ...
            if mask.sum() > 0:
                q_mae  = np.mean(abs_errors[mask])
                q_rmse = np.sqrt(np.mean(errors[mask] ** 2))
    ...
    for i in range(len(all_gt_counts)):
        print(f"{i+1:<8} {all_gt_counts[i]:>6.0f} {all_pred_counts[i]:>8.1f} "
              f"{errors[i]:>+8.1f}")
...
def _plot_pred_vs_gt(gt_counts, pred_counts, results, output_dir):
    """Publication-quality scatter + residual plot."""
    ...
    # ---- Left panel: Predicted vs Ground Truth ----
    ax = axes[0]

    # Color points by absolute error magnitude
    abs_err = np.abs(pred_counts - gt_counts)
    sc = ax.scatter(gt_counts, pred_counts,
                    c=abs_err, cmap='YlOrRd', s=60, alpha=0.85,
    ...
    if len(gt_counts) > 2:
        ...
        # Approx CI: ±t * se * sqrt(1/n + (x-xbar)^2/Sxx)
        n = len(gt_counts)
        x_bar = gt_counts.mean()
        ...
        ax.plot(x_fit, y_fit, color='#2166ac', linewidth=1.5, label=f'OLS (slope={slope:.2f})')
    ...
    # ---- Right panel: Residuals vs Ground Truth ----
    ax2 = axes[1]
    residuals = pred_counts - gt_counts

    ax2.scatter(gt_counts, residuals,
                c=abs_err, cmap='YlOrRd', s=60, alpha=0.85,
    ...
    ax2.set_title('Residual Plot\n(systematic bias visible as trend)',
                  fontsize=12, fontweight='bold')
